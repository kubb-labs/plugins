...
This requires a `kubb` release that includes the per-node cache from kubb-labs/kubb#3812 (`ctx.cache` / `NodeCache`, merged to main but not yet published as of this changeset). Bump the `kubb` pin in `pnpm-workspace.yaml` once that release ships.
