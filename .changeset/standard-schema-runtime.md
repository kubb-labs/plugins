...
Generated clients pass the schema straight to the `parser` slot instead of wrapping it in a `.parse(data)` call. The `parser` slot now takes a Standard Schema validator, and only `client.ts` calls `validateStandardSchema`, so the helper stays in one place instead of being imported into every operation file.
