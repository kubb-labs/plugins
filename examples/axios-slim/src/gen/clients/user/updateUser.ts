/**
* Generated by Kubb (https://kubb.dev/).
* Do not edit manually.
*/

import type { Options, RequestResult } from '../../.kubb/client.ts'
import type { UpdateUserRequestConfig, UpdateUserResponses } from '../../models/user/UpdateUser.ts'
import { client } from '../../.kubb/client.ts'

/**
 * @description This can only be done by the logged in user.
 * @summary Update user
 * {@link /user/:username}
 */
export function updateUser<ThrowOnError extends boolean = true>(options: Options<UpdateUserRequestConfig, ThrowOnError>): Promise<RequestResult<UpdateUserResponses, ThrowOnError>> {
  const { client: request = client, ...config } = options

  return request({ method: 'PUT', url: '/user/{username}', ...config }) as Promise<RequestResult<UpdateUserResponses, ThrowOnError>>
}