/**
* Generated by Kubb (https://kubb.dev/).
* Do not edit manually.
*/

import type { Options, RequestResult } from '../../.kubb/client.ts'
import type { LogoutUserRequestConfig, LogoutUserResponses } from '../../models/user/LogoutUser.ts'
import { client } from '../../.kubb/client.ts'

/**
 * @summary Logs out current logged in user session
 * {@link /user/logout}
 */
export function logoutUser<ThrowOnError extends boolean = true>(options: Options<LogoutUserRequestConfig, ThrowOnError>): Promise<RequestResult<LogoutUserResponses, ThrowOnError>> {
  const { client: request = client, ...config } = options

  return request({ method: 'GET', url: '/user/logout', ...config }) as Promise<RequestResult<LogoutUserResponses, ThrowOnError>>
}