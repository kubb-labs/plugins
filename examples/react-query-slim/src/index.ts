...
// Configure the slim runtime the generated React Query hooks call. There is no runtime dependency on
// @kubb/plugin-fetch. The hooks return the response body and surface `ResponseError` (typed as
// `ResponseErrorConfig`) on `useQuery().error`, with `error.response` reachable.
