...
import type { GetOrderByIdResponse, GetOrderByIdPathOrderId, GetOrderByIdStatus200, GetOrderByIdStatus400, GetOrderByIdStatus404 } from '../../models/GetOrderById.ts'
...
export function getOrderByIdQueryOptions({ orderId }: { orderId?: GetOrderByIdPathOrderId } = {}, config: Partial<RequestConfig> & { client?: Client } = {}) {
  return {
    fetcher: async () => {
      return getOrderById({ orderId: orderId! }, config)
    },
  }
}
 ...
 * {@link /store/order/:orderId}
 */
export function useGetOrderById({ orderId }: { orderId?: GetOrderByIdPathOrderId } = {}, options: {
