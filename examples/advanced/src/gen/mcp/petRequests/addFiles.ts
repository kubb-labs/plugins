...
export async function addFilesHandler({ data }: { data: AddFilesData }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<AddFilesResponse, ResponseErrorConfig<AddFilesStatus405>, AddFilesData>({ method: "POST", url: `/pet/files`, baseURL: `https://petstore.swagger.io/v2`, data: formData as FormData })
