...
export async function getPetByIdHandler({ petId }: { petId: GetPetByIdPathPetId }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<GetPetByIdResponse, ResponseErrorConfig<GetPetByIdStatus400 | GetPetByIdStatus404>, unknown>({ method: "GET", url: `/pet/${petId}:search`, baseURL: `https://petstore.swagger.io/v2` })
