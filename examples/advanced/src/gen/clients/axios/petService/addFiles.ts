...
export async function addFiles({ data }: { data: AddFilesData }, config: Partial<RequestConfig<AddFilesData>> & { client?: Client } = {}) {
  ...
  const res = await request<AddFilesResponse, ResponseErrorConfig<AddFilesStatus405>, AddFilesData>({ method: "POST", url: getAddFilesUrl().url.toString(), data: formData as FormData, ...requestConfig })
