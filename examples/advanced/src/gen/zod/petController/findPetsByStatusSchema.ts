...
export const findPetsByStatusStatus200Schema = z.array(petSchema).min(1).max(3).refine(items => new Set(items).size === items.length, { message: "Array entries must be unique" })
