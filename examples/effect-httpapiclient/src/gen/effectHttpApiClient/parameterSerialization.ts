...
function deserialize({ value, options }: { value: unknown; options: ParameterOptions }): unknown {
  ...
  if (options.kind === 'array') {
    const delimiter =
      options.style === 'spaceDelimited' ? ' ' : options.style === 'pipeDelimited' ? '|' : options.style === 'label' && options.explode ? '.' : ','
...
export function queryParameter<S extends Schema.Constraint>(schema: S, options: ParameterOptions) {
  const encoded = Schema.Union([Schema.String, Schema.Array(Schema.String), Schema.Record(Schema.String, Schema.String)])
