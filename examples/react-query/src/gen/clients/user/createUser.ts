/**
* Generated by Kubb (https://kubb.dev/).
* Do not edit manually.
*/

import type { Options, RequestResult } from '../../.kubb/client.ts'
import type { CreateUserRequestConfig, CreateUserResponses } from '../../models/user/CreateUser.ts'
import { client } from '../../.kubb/client.ts'

/**
 * @description This can only be done by the logged in user.
 * @summary Create user
 * {@link /user}
 */
export function createUser<ThrowOnError extends boolean = true>(options: Options<CreateUserRequestConfig, ThrowOnError>): Promise<RequestResult<CreateUserResponses, ThrowOnError>> {
  const { client: request = client, ...config } = options

  return request({ method: 'POST', url: '/user', ...config }) as Promise<RequestResult<CreateUserResponses, ThrowOnError>>
}