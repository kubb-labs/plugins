 ...
 * {@link /user/:username}
 */
export async function updateUser({ username }: { username: UpdateUserPathParams["username"] }, data?: UpdateUserMutationRequest, config: Partial<RequestConfig<UpdateUserMutationRequest>> & { client?: Client } = {}) {
  ...
  const res = await request<UpdateUserMutationResponse, ResponseErrorConfig<Error>, UpdateUserMutationRequest>({ method : "PUT", url : `/user/${username}`, data : requestData, ... requestConfig })
