...
export async function addPet(data: AddPetMutationRequest, config: Partial<RequestConfig<AddPetMutationRequest>> & { client?: Client } = {}) {
  ...
  const res = await request<AddPetMutationResponse, ResponseErrorConfig<AddPet405>, AddPetMutationRequest>({ method : "POST", url : `/pet`, data : requestData, ... requestConfig })
...
export function useAddPet<TContext>(options: {
  mutation?: MutationObserverOptions<AddPetMutationResponse, ResponseErrorConfig<AddPet405>, {data: MaybeRefOrGetter<AddPetMutationRequest>}, TContext> & { client?: QueryClient },
