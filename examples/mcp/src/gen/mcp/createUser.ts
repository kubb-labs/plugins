...
export async function createUserHandler({ data }: { data?: CreateUserData } = {}, request: RequestHandlerExtra<ServerRequest, ServerNotification>): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<CreateUserResponse, ResponseErrorConfig<Error>, CreateUserData>({ method: "POST", url: `/user`, baseURL: `https://petstore.swagger.io/v2`, data: requestData }, request)
