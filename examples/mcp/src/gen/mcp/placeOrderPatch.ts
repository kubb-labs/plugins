...
export async function placeOrderPatchHandler({ data }: { data?: PlaceOrderPatchData } = {}): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<PlaceOrderPatchResponse, ResponseErrorConfig<PlaceOrderPatchStatus405>, PlaceOrderPatchData>({ method: "PATCH", url: `/store/order`, baseURL: `https://petstore.swagger.io/v2`, data: requestData })
