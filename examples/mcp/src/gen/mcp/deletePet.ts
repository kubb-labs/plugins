...
export async function deletePetHandler({ petId, headers }: { petId: DeletePetPathPetId; headers?: { api_key?: DeletePetHeaderApiKey } }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<DeletePetResponse, ResponseErrorConfig<DeletePetStatus400>, unknown>({ method: "DELETE", url: `/pet/${petId}`, baseURL: `https://petstore.swagger.io/v2`, headers: { ...headers } })
