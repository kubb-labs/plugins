...
export async function deleteOrderHandler({ orderId }: { orderId: DeleteOrderPathOrderId }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<DeleteOrderResponse, ResponseErrorConfig<DeleteOrderStatus400 | DeleteOrderStatus404>, unknown>({ method: "DELETE", url: `/store/order/${orderId}`, baseURL: `https://petstore.swagger.io/v2` })
