...
export async function placeOrderHandler({ data }: { data?: PlaceOrderData } = {}): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<PlaceOrderResponse, ResponseErrorConfig<PlaceOrderStatus405>, PlaceOrderData>({ method: "POST", url: `/store/order`, baseURL: `https://petstore.swagger.io/v2`, data: requestData })
