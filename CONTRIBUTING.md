...
3. The `promote` job then verifies the versions are actually live on npm, tags the released versions, creates a GitHub Release, and only then dispatches the content update to [kubb-labs/platform](https://github.com/kubb-labs/platform).

Each plugin versions independently here, so `promote` creates one GitHub Release per package, with notes pulled from that package's own `CHANGELOG.md`.
