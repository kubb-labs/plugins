...
Packages in this repo version and changelog independently (see the empty `fixed` and `linked` groups in `.changeset/config.json`), so a release here creates one GitHub Release per staged package, tagged `<package>@<version>`, with notes taken from that package's own `CHANGELOG.md`. This differs from [kubb-labs/kubb](https://github.com/kubb-labs/kubb), where every package shares one fixed version and a release covers all of them combined. The `release` and `promote` steps come from the shared [kubb-labs/config](https://github.com/kubb-labs/config) actions (`.github/actions/release` and `.github/actions/promote`); this repo simply never sets the `promote` action's `release-mode` input to `combined`.
