...
import { ast, type NormalizedPlugin } from '@kubb/core'
...
export async function generatePerPluginBarrel(
  ...
  const barrelFiles = await getBarrelFiles(files as Array<ast.FileNode>, {
    type: barrelType,
    root,
    output,
    meta: { pluginName },
...
export async function generateRootBarrel(
  ...
  const allFiles = files as Array<ast.FileNode>
  ...
  const rootFile = ast.createFile<object>({
    path: rootPath,
