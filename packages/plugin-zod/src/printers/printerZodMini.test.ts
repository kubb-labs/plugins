...
describe('printerZodMini', () => {
  ...
  describe('string', () => {
    ...
    test('string with length checks', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'string', min: 1, max: 10 }))).toBe('z.string().check(z.minLength(1), z.maxLength(10))')
    })

    test('string with pattern', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'string', pattern: '^\\d+$' }))).toBe('z.string().check(z.regex(/^\\d+$/))')
    })
  ...
  describe('number', () => {
    ...
    test('number with numeric checks', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'number', min: 0, max: 100 }))).toBe('z.number().check(z.minimum(0), z.maximum(100))')
    })

    test('number with multipleOf', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'number', multipleOf: 5 }))).toBe('z.number().check(z.multipleOf(5))')
    })

    test('integer with min, max, and multipleOf', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'integer', min: 0, max: 100, multipleOf: 10 }))).toBe(
  ...
  describe('integer', () => {
    ...
    test('integer with int32 format and checks', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'integer', format: 'int32', min: 0, max: 100 }))).toBe(
        'z.int32().check(z.minimum(0), z.maximum(100))',
      )
    })
  })
  ...
  describe('date types', () => {
    test('date (JS Date)', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'date', representation: 'date' }))).toBe('z.date()')
    })
    ...
    test('datetime falls back to z.string()', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'datetime' }))).toBe('z.string()')
    })
  ...
  describe('special string formats', () => {
    ...
    test('format helpers with checks', () => {
      expect(printer.print(ast.factory.createSchema({ type: 'string', format: 'base64', min: 10 }))).toBe('z.base64().check(z.minLength(10))')
    })
  })
  ...
  describe('modifiers (functional syntax)', () => {
    ...
    test('nullable and optional wraps both', () => {
      const node = ast.factory.createSchema({ type: 'string', nullable: true, optional: true })
  ...
  describe('default (functional syntax)', () => {
    test('string default', () => {
      const node = ast.factory.createSchema({ type: 'string', default: 'hello' })
      expect(printer.print(node)).toMatchInlineSnapshot(`"z._default(z.string(), 'hello')"`)
    })

    test('number default', () => {
      const node = ast.factory.createSchema({ type: 'number', default: 42 })
  ...
  describe('ref', () => {
    ...
    test('object indirect-cycle property uses getter with bare schema name (no double z.lazy)', () => {
      ...
      const p = printerZodMini({ cyclicSchemas: new Set(['Cat', 'Pet']) })
      const node = ast.factory.createSchema({
        type: 'object',
        primitive: 'object',
        properties: [
  ...
  describe('union', () => {
    ...
    test('discriminated union flattens allOf variants with z.extend() so they discriminate', () => {
      ...
      expect(printer.print(node)).toBe(`z.discriminatedUnion('petType', [
  Cat,
  z.extend(BasePet, {
    petType: z.string(),
  }),
  ...
  describe('nodes override', () => {
    test('overrides a single node type', () => {
      ...
      expect(p.print(ast.factory.createSchema({ type: 'date', representation: 'string' }))).toBe('z.iso.date()')
    })
    ...
    test('override can read this.options', () => {
      ...
      expect(p.print(ast.factory.createSchema({ type: 'uuid' }))).toBe('z.guid().custom()')
    })
  })
})
