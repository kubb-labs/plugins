...
export type Options = {
  ...
  /**
   * Reference base schema types directly instead of the per-operation aliases generated by
   * `@kubb/plugin-ts`. When `false`, a `$ref`-backed request or 2xx/error response uses the
   * base component type (e.g. `Pet`) in the generated function signature.
   *
   * @default true
   * @note Set the same value on `@kubb/plugin-ts` and any other plugin that references
   * operation types so the generated names stay consistent.
   */
