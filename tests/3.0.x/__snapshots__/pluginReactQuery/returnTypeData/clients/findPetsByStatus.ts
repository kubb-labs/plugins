...
export function findPetsByStatus<ThrowOnError extends boolean = true>(options: Options<FindPetsByStatusOptions, ThrowOnError> = {}): Promise<UnwrappedResult<FindPetsByStatusResponses, ThrowOnError>> {
  ...
  return unwrapResult(request({ method: 'GET', url: '/pet/findByStatus', security: [{ type: 'oauth2' }], styles: { query: { status: { explode: true } } }, ...config, throwOnError: config.throwOnError ?? true }), config.throwOnError ?? true) as Promise<UnwrappedResult<FindPetsByStatusResponses, ThrowOnError>>
}
