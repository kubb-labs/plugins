...
afterAll(() => {
  ...
  console.log('\n  plugin-ts only:')
  row('  adapter + framework', baseline, `  [${(baseline / streamTs * 100).toFixed(0)}% of stream total]`)
  row('  generators+JSX+print (stream)', streamTs - baseline, `  [${((streamTs - baseline) / streamTs * 100).toFixed(0)}% of stream total]`)
  row('  generators+JSX+print (batch)',  batchTs  - baseline, `  [${((batchTs  - baseline) / batchTs  * 100).toFixed(0)}% of batch total]`)
  row('  streaming speedup', batchTs - streamTs, `  [${(batchTs / streamTs).toFixed(2)}× batch/stream]`)

  console.log('\n  plugin-ts + plugin-zod:')
  row('  adapter + framework', baseline, `  [${(baseline / streamTsZod * 100).toFixed(0)}% of stream total]`)
  row('  generators+JSX+print (stream)', streamTsZod - baseline, `  [${((streamTsZod - baseline) / streamTsZod * 100).toFixed(0)}% of stream total]`)
  row('  generators+JSX+print (batch)',  batchTsZod  - baseline, `  [${((batchTsZod  - baseline) / batchTsZod  * 100).toFixed(0)}% of batch total]`)
  row('  plugin-zod marginal (stream)',  streamTsZod - streamTs)
  row('  plugin-zod marginal (batch)',   batchTsZod  - batchTs)
  row('  streaming speedup', batchTsZod - streamTsZod, `  [${(batchTsZod / streamTsZod).toFixed(2)}× batch/stream]`)
  console.log()
})
