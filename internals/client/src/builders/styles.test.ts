...
import { buildStylesMetadata } from './styles.ts'
...
describe('buildStylesMetadata', () => {
  test('returns null when no parameter carries style or explode', () => {
    ...
    expect(buildStylesMetadata({ node })).toBeNull()
  })
  ...
  test('emits path style and explode keyed by the camelCased name', () => {
    ...
    expect(buildStylesMetadata({ node })).toBe("{ path: { petId: { style: 'matrix', explode: true } } }")
  })
  ...
  test('emits query style and explode', () => {
    ...
    expect(buildStylesMetadata({ node })).toBe("{ query: { tags: { style: 'pipeDelimited', explode: false } } }")
  })
  ...
  test('groups multiple locations together', () => {
    ...
    expect(buildStylesMetadata({ node })).toBe("{ path: { id: { style: 'label', explode: false } }, query: { fields: { explode: true } } }")
  })
})
